...
def exponential_decay(learning_rate, global_steps, decay_steps, decay_rate, staircase=False, name=None):
    return learning_rate * np.power(decay_rate, (global_steps / decay_steps))
...
learning_rate = exponential_decay(0.0001, global_step, 10000, 0.98)
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        if (epoch + 1) % display_step == 0:
            ...
            print('Epoch', '%04d' % (epoch+1), 'learning_rate =', rate, 'cost =', avg_cost, 'accu =', accurary.eval({inputDict['x']: x_test, inputDict['y']: y_test}))
