...
color = ['r' if l ==0 else 'b' for l in Y[:]]
...
with tf.Session() as sess:
    ...
    for epoch in range(maxEpochs):
        ...
        for i in range(np.int32(len(Y) / minibatchSize)):
            ...
            _, lossval,outputval, errval = sess.run([train, loss, output, err], feed_dict={input_feature:X1, input_labels:Y1})
            sumerr += (errval / minibatchSize)
        print('Epoch:', '%04d' % (epoch+1), 'cose=', lossval, 'err=', sumerr / minibatchSize)