...
with tf.Session() as sess:
    ...
    summary_writer = tf.summary.FileWriter(os.path.join('log', 'summaries'), sess.graph)

    plotdata = {'batchsize':[], 'loss':[], 'avgloss':[]}
    for epoch in range(training_epochs):
        for (x, y) in zip(train_X, train_Y):
            sess.run(optimizer, feed_dict={inputdict['x']:x, inputdict['y']:y})
            summary_str = sess.run(merged_summary_op, feed_dict={inputdict['x']:x, inputdict['y']:y})
